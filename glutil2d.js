...
function createWebGLContext(canvas) {
   ...
   let gl = c.getContext("webgl", { depth: false, antialias: true } );
   if ( ! gl ) {
...
function Animator(callback) {
   ...
   let timePaused = 0;
   let pauseStart;
   const reqAnimFrm = window.requestAnimationFrame || 
        window.mozRequestAnimationFrame ||
        window.webkitRequestAnimationFrame ||
        window.msRequestAnimationFrame ||
        (frame => {
           setTimeout(() => { frame(Date.now()); },  1000/60);
        });
   function doFrm( time ) {
      if (!animating) {
          pauseStart = time;
          ...
          const wallTime = time - animationStartTime;   // Time since start of animation.
          const animationTime = wallTime - timePaused;  // Time during whigh animation was running (not counting pauses).
...
function AffineTransform2D() {
   if (arguments.length == 0) {
      ...
      const src = (arguments.length == 1 && arguments[0] instanceof Array) ? arguments[0] : arguments;
...
AffineTransform2D.prototype.times = function( /* AffineTransform2D */ that ) {
   const a = this.a*that.a + this.c*that.b;
   const b = this.b*that.a + this.d*that.b;
   const c = this.a*that.c + this.c*that.d;
   const d = this.b*that.c + this.d*that.d;
   const e = this.a*that.e + this.c*that.f + this.e;
   const f = this.b*that.e + this.d*that.f + this.f;
   this.a = a;
   this.b = b;
   this.c = c;
   this.d = d;
